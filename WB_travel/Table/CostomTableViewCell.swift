...
class CustomTableViewCell: UITableViewCell {
    ...
    @objc private func checkLikeAction() {
        if self.isCheked ?? false {
            checkButton.setImage(UIImage(systemName: "heart.fill"), for: .normal)
            self.isCheked?.toggle()
        } else {
            checkButton.setImage(UIImage(systemName: "heart"), for: .normal)
            self.isCheked?.toggle()
        }
    ...
    func setupCell(_ cell: CustomCell) {
        ...
        self.checkButton.setImage(UIImage(systemName: cell.isLike ? "heart.fill" : "heart"), for: .normal)
        self.idCell = cell.id
    }
